(function(){
  ...
  */
  ...
  function HashMismatchException(message,reason) {
      this.name="HashMismatchException";
      this.message=(message||"");
      this.reason=(reason||"");
  }

  function insert(doc,src,type){
    (doc.getElementsByTagName('HEAD')[0]||doc.body)
      .appendChild(doc.createElement(type))
      .appendChild(doc.createTextNode(src));
  }

  /*Returns an object which is the public interface.*/
  return{
    loadScript:function(src,hash,doc){
      var srcHash=CryptoJS.SHA256(src).toString(CryptoJS.enc.Base64Url);
      if(hash!==srcHash){
        throw new HashMismatchException("SecLoad.loadScript(): Unexpected hash","Calculated hash is: "+srcHash);
      }
      insert(doc,src,"script");
      return this;
    },
    loadStyle:function(src,hash,doc){
      var srcHash=CryptoJS.SHA256(src).toString(CryptoJS.enc.Base64Url);
      if(hash!==srcHash){
        throw new HashMismatchException("SecLoad.loadStyle(): Unexpected hash","Calculated hash is: "+srcHash);
      }
      insert(doc,src,"style");
      return this;
    }
  };
})();
